...
function cobalt_download($video_url)
{
    ...
    $ch2 = curl_init($fallback_api_url . '/download');
    ...
    $redirect_url = curl_getinfo($ch2, CURLINFO_EFFECTIVE_URL);
    curl_close($ch2);

    if ($http_code2 === 200 && $res2 !== false) {
        // yt-dlp API fayl qaytaradi — uni Telegram ga yuborish uchun URL kerak
        // Effective URL ni qaytaramiz (redirect bo'lgan bo'lsa)
        return $redirect_url ?? null;
    }

    error_log("Fallback API ham ishlamadi. HTTP: $http_code2");
    return null;
}
