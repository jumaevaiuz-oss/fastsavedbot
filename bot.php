...
// ⚙️ Sozlash bo'limi
if ($text == "⚙️ Sozlash" and admin($cid) == 1) {
    sms($cid, "<b>⚙️ Sozlash bo'limi</b>\n\nKerakli bo'limni tanlang:", $sozlash_panel);
    exit();
}

// 🗄 Boshqarish (sozlash bo'limidan qaytish)
if ($text == "🗄 Boshqarish" and admin($cid) == 1) {
    sms($cid, "<b>🗄 Admin panel</b>", $panel);
    step($cid, "none");
    exit();
}

// 🔗 API URL bo'limi
if ($text == "🔗 API URL" and admin($cid) == 1) {
    $res = mysqli_query($connect, "SELECT cobalt_url, fallback_url FROM settings WHERE id = 1");
    $row = mysqli_fetch_assoc($res);
    $cobalt_url   = $row['cobalt_url'] ?? null;
    $fallback_url = $row['fallback_url'] ?? null;

    $msg = "<b>🔗 API URL sozlamalari</b>
...
";
    ...
    if ($cobalt_url) {
        $msg .= "🟢 <b>Cobalt URL:</b>
<code>$cobalt_url</code>

";
    } else {
        $msg .= "🔴 <b>Cobalt URL:</b> mavjud emas
...
";
    ...
    // Fallback URL
    if ($fallback_url) {
        $msg .= "🟢 <b>Fallback URL:</b>
<code>$fallback_url</code>

";
    } else {
        $msg .= "🔴 <b>Fallback URL:</b> mavjud emas
...
";
    ...
    $msg .= "<b>Buyruqlar:</b>
";
    $msg .= "/new https://url — Cobalt URL qo'shish
";
    $msg .= "/update https://url — Cobalt URL yangilash
";
    $msg .= "/updatefallback https://url — Fallback URL yangilash
";
    $msg .= "/del — Cobalt URL o'chirish";

    sms($cid, $msg, $sozlash_panel);
    exit();
}

// /new Cobalt URL qo'shish
if (isset($tx) && mb_strpos((string)$tx, "/new ") === 0 and admin($cid) == 1) {
    $new_url = trim(str_replace("/new ", "", $tx));
    if (filter_var($new_url, FILTER_VALIDATE_URL)) {
        $stmt = $connect->prepare("UPDATE settings SET cobalt_url = ? WHERE id = 1");
        $stmt->bind_param("s", $new_url);
        $stmt->execute();
        $stmt->close();
        sms($cid, "✅ <b>Cobalt URL qo'shildi:</b>
<code>$new_url</code>", $sozlash_panel);
    } else {
        sms($cid, "❌ <b>Noto'g'ri URL format!</b>
Misol: /new https://cobalt.railway.app", $sozlash_panel);
    }
    exit();
}

// /update Cobalt URL yangilash
if (isset($tx) && mb_strpos((string)$tx, "/update ") === 0 and admin($cid) == 1) {
    $new_url = trim(str_replace("/update ", "", $tx));
    if (filter_var($new_url, FILTER_VALIDATE_URL)) {
        $stmt = $connect->prepare("UPDATE settings SET cobalt_url = ? WHERE id = 1");
        $stmt->bind_param("s", $new_url);
        $stmt->execute();
        $stmt->close();
        sms($cid, "✅ <b>Cobalt URL yangilandi:</b>
<code>$new_url</code>", $sozlash_panel);
    } else {
        sms($cid, "❌ <b>Noto'g'ri URL format!</b>
Misol: /update https://cobalt.railway.app", $sozlash_panel);
    }
    exit();
}

// /updatefallback Fallback URL yangilash
if (isset($tx) && mb_strpos((string)$tx, "/updatefallback ") === 0 and admin($cid) == 1) {
    $new_url = trim(str_replace("/updatefallback ", "", $tx));
    if (filter_var($new_url, FILTER_VALIDATE_URL)) {
        $stmt = $connect->prepare("UPDATE settings SET fallback_url = ? WHERE id = 1");
        $stmt->bind_param("s", $new_url);
        $stmt->execute();
        $stmt->close();
        sms($cid, "✅ <b>Fallback URL yangilandi:</b>
<code>$new_url</code>", $sozlash_panel);
    } else {
        sms($cid, "❌ <b>Noto'g'ri URL format!</b>
Misol: /updatefallback https://ytdlp.railway.app", $sozlash_panel);
    }
    exit();
}

// /del Cobalt URL o'chirish
if ($tx == "/del" and admin($cid) == 1) {
    $connect->query("UPDATE settings SET cobalt_url = NULL WHERE id = 1");
    sms($cid, "🗑️ <b>Cobalt URL o'chirildi</b>
Fallback URL ishlashda davom etadi.", $sozlash_panel);
    exit();
}
