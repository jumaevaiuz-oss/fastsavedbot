...
if ($text == "🔗 API URL" and admin($cid) == 1) {
    $res = mysqli_query($connect, "SELECT cobalt_url FROM settings WHERE id = 1");
    $row = mysqli_fetch_assoc($res);
    $cobalt_url = $row['cobalt_url'] ?? null;

    if ($cobalt_url) {
        sms($cid, "<b>🔗 Mavjud URL:</b>\n<code>$cobalt_url</code>\n\n<b>Buyruqlar:</b>\n/update https://yangi-url.railway.app — yangilash\n/del — o'chirish", $sozlash_panel);
    } else {
        sms($cid, "<b>🔗 Hozir URL mavjud emas</b>\n\n<b>Buyruq:</b>\n/new https://server-url.railway.app", $sozlash_panel);
    }
    exit();
}
...
if ($tx == "/del" and admin($cid) == 1) {
    ...
    sms($cid, "🗑️ <b>URL o'chirildi</b>", $sozlash_panel);
    exit();
}
